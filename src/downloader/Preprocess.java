...
public class Preprocess implements DownloadMod, AutoCloseable {
	...
	public void connector(String rawAddress, int downloadMode, final BufferedReader in) {
		...
		switch (downloadMode) {
			...
			case SELECTIVE_DOWNLOAD: //선택적 다운로드 기능
				try {
					if (archiveAddrList.size() == 0) {
						throw new RuntimeException("잘못된 페이지입니다: " + rawAddress);
					}

					showList(archiveAddrList); //다운 가능한 페이지 출력
					print.info("다운받을 번호를 입력하세요: ");
	...
	private void showList(final List<Comic> archiveAddrList) {
		...
		for (Comic comic : archiveAddrList) {
			print.info("{}\n", String.format("%3d. %s", idx++, comic.getTitle()));
		}
	}
	...
	private List<Integer> parse(final String COMMAND, final int MAX_PAGE) {
		...
		try {
			...
			if (pages.size() == 0) {
				throw new RuntimeException("페이지 선택 형식이 잘못되었습니다.");
			}
		} catch (Exception e) {
